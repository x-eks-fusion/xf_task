...
该例程主要展示 ctask 和 ttask 被事件触发执行的效果。

该例程创建了三个任务 ctask 和 ttask 任务。还有一个 trigger 任务用于发出触发。
当使用 xf_ttask_create_loop() 创建任务，且创建任务的时间设置为 0 的时候。说明该任务不能被时间触发，只能通过 trigger 触发。
